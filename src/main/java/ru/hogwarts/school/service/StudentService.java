...
public class StudentService {
    ...
    public Student findStudent(Long id) {
        logger.debug("Вызван метод - найти студента с номером {}.", id);
    ...
    public void printStudentsName() {
       List<Student> studentList = studentRepository.findAll(PageRequest.of(0, 6)).getContent();

        printStudents(studentList.subList(0, 2));
        new Thread(() -> printStudents(studentList.subList(2, 4))).start();
        new Thread(() -> printStudents(studentList.subList(4, 6))).start();

    }
    public void printStudentsNameSynchr() {
        List<Student> studentList = studentRepository.findAll(PageRequest.of(0, 6)).getContent();

        printStudentsSynchr(studentList.subList(0, 2));
        new Thread(() -> printStudentsSynchr(studentList.subList(2, 4))).start();
        new Thread(() -> printStudentsSynchr(studentList.subList(4, 6))).start();}
    ...
    private synchronized void printStudentsSynchr(List<Student> students) {
        for (Student s : students) {
            System.out.println(s.getName());
        }
    }
